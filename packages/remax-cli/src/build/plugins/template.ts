...
async function createTemplate(
  ...
  if (meta.jsHelper) {
    renderOptions.jsHelper = winPath(
      path.relative(path.dirname(pageFile), `helper${meta.jsHelper.extension}`)
    );
  }
...
async function createHelperFile(meta: Meta) {
  ...
  return {
    type: 'asset' as const,
    fileName: `helper${meta.jsHelper.extension}`,
    source: code,
  };
}
...
export default function template(
  ...
  return {
    ...
    async generateBundle(_, bundle, isWrite) {
      ...
      const helperFile = await createHelperFile(meta);
      if (helperFile) {
